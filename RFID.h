...
{
  public:
    // 软件SPI构造函数：CS引脚、复位引脚、SCK引脚、MOSI引脚、MISO引脚
  ...
  private:
	int _chipSelectPin;  // CS片选引脚
	int _NRSTPD;         // 复位引脚
	int _sckPin;         // 软件SPI时钟引脚
	int _mosiPin;        // 软件SPI数据输出引脚（主机→从机）
	int _misoPin;        // 软件SPI数据输入引脚（从机→主机）
	
	// 软件SPI核心函数：发送一个字节并返回接收的字节
